 ...
 * Module dependencies.
 */
// var Promise = require('./QPromise').Promise;
...
Promise.prototype.unstore = function() {
...
Promise.prototype.one = function() {
...
Promise.prototype.set = function(path, value) {
  ...
  // return this.then(function(ret) {
    var p = path.split('.');
    var f = p.pop(), o = this, self = this, o = this;
...
APIClient._createPromise = APIClient.prototype._createPromise = function(name) {
  ...
  Object.defineProperty(Promise.prototype, name, {
    get: function() {
      ...
      var props = Object.getOwnPropertyNames(Promise.prototype);
      // console.log('propsss', props);
      for (var a = 0, l = props.length; a < l; a++) {
...
APIClient._createPromise('filter');

APIClient.prototype._getLink = function(obj, link) {
  return this._routes[obj._type + 's'][link].href.replace(/\{([\.\w]+)\}/g, function(match, what) {
    var dat = LINK_EXTRACT_REGX.exec(what);
...
APIClient.prototype._extractLinks = function(links) {
  ...
  for (var link in links) {
    var dat = LINK_EXTRACT_REGX.exec(link);
    if (!dat) {
      continue;
    }
    ...
    key = dat[1];
    value = dat[2];
    this._routes[key] = this._routes[key] || {};

    if (typeof links[link] == 'string') {
      route = this._routes[key][value] = {
        'method': 'GET',
        'href': links[link]
      };
    } else if (typeof links[link] == 'object') {
      route = this._routes[key][value] = links[link];
      route.fields = route.fields || {};
      for (var n in route.fields) {
        if (!Array.isArray(route.fields[n])) route.fields[n] = [route.fields[n]];
      }
    }

    if (route.fields) {
      this._createInstance(inflection.singularize(key), {})._addAction(value);
    } else {
      this._createPromise(value);
    }
  }
};


APIClient.prototype.get = function(path, qs, list) {
  if (list) {
    return this.list(path, qs || {});
  }

  return this._request('GET', path, qs || {});
};

APIClient.prototype.list = function(path) {
  return q(new Page(this, path));
};
